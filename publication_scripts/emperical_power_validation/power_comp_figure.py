...
# Function creating lighter version of a color for plot
...
for path in mat_files:
    ...
    # (top-QUANTILE% of edges ranked directly by tpr magnitude)
    m = tpr.size
    k = max(1, int(round(m * QUANTILE / 100.0)))
    order = np.argsort(tpr)[::-1]  # tpr in [0,1], no abs needed
    slice_idx = order[:k]
    avg_power = np.mean(tpr[slice_idx])

    power_dict.setdefault(key, dict())
...
for i, gt_key in enumerate(gt_json):
    ...
    for n_subs in ns:
        ...
        if n_key not in gt_json[gt_key] or q_key not in gt_json[gt_key][n_key]:
            raise ValueError('Gt json does not contain subnumber or q_key')
